...
const NAMED_EMPLOYEES: EmployeeSeed[] = [
  ...
  {
    code: 'EMP1088', name: 'Rohan Mehta', role: 'Backend Engineer',
  ...
  {
    code: 'EMP1121', name: 'Sara Lin', role: 'Full-Stack Engineer',
  ...
  {
    code: 'EMP1155', name: 'Kabir Anand', role: 'Platform Engineer',
  ...
  {
    code: 'EMP1190', name: 'Meena Pillai', role: 'QA Automation Engineer',
  ...
  {
    code: 'EMP1204', name: 'Devika Nair', role: 'Backend Engineer',
  ...
  {
    code: 'EMP1233', name: 'Farah Iqbal', role: 'Data Engineer',
  ...
  {
    code: 'EMP1267', name: 'Tomás Silva', role: 'Mobile Engineer',
...
async function main(): Promise<void> {
  ...
  const prince = await prisma.user.create({
    data: { name: 'Prince Verma', email: 'prince.verma@fortis.demo', passwordHash, role: 'manager', title: 'Delivery Manager · Bengaluru' },
  });
  await prisma.user.create({
    data: { name: 'Soumyadeep', email: 'soumyadeep@fortis.demo', passwordHash, role: 'hr', title: 'Head of Resourcing · Gurugram' },
  });
  const satyaUser = await prisma.user.create({
    data: { name: 'Satya Mishra', email: 'satya.mishra@fortis.demo', passwordHash, role: 'candidate', title: 'Senior Software Engineer', employeeId: employeeMap['EMP1042'] },
  });
  const devraj = await prisma.user.create({
    data: { name: 'Devraj Rao', email: 'devraj.rao@fortis.demo', passwordHash, role: 'manager', title: 'Delivery Manager · Hyderabad' },
  });
  console.log('  ✓ 4 users');

  // 4. Projects
  const proj1 = await prisma.project.create({ data: { name: 'Payments Platform — Phase 2', customer: 'Northwind Financial', managerId: prince.id, status: 'Active', startDate: new Date('2025-10-01'), tags: ['Payments', 'FinTech'] } });
  const proj2 = await prisma.project.create({ data: { name: 'Claims Automation Revamp', customer: 'Meridian Health', managerId: prince.id, status: 'Active', startDate: new Date('2025-12-01'), tags: ['Healthcare', 'Automation'] } });
  const proj3 = await prisma.project.create({ data: { name: 'Logistics Tracker API', customer: 'Atlas Freight', managerId: devraj.id, status: 'Active', startDate: new Date('2026-01-15'), tags: ['Logistics'] } });
  const proj4 = await prisma.project.create({ data: { name: 'Retail Loyalty Engine', customer: 'Corestone Retail', managerId: devraj.id, status: 'On hold', startDate: new Date('2026-03-01'), tags: ['Retail', 'Loyalty'] } });
  ...
  console.log('\n✅ Seed complete — 65 employees, 4 projects, 5 IRCs, 3 pipeline entries ready.');
}
